...
namespace TriadCompiler.Parser.Common.Var
{
    ...
    internal class Variable : CommonParser
    {
        ...
        public static VarInfo Parse(EndKeyList endKeys, bool allowRange, bool allowNotIndexedArray)
        {
            ...
            //для инфПроцедур, если обращение к spyпеременной
            //еще надо для set метода подобное сделать, но пока нет
            //но возможно это лажа
            if (varInfo.Type!=null && varInfo.Type.IsSpyObject)
            {
                if (objRef.StrCode.Contains("["))
                {
                    objRef.StrCode=objRef.StrCode.Replace('[', '(');
                    objRef.StrCode=objRef.StrCode.Replace(']', ')');
                }
                else
                    objRef.StrCode += "()";
                varInfo.StrCode = "get" + objRef.StrCode;
