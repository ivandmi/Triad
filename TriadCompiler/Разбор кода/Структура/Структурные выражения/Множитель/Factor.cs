...
using TriadCompiler.Parser.Structure.StructExpr.RandGraph;
        ...
        public static CodeStatementCollection Parse(EndKeyList endKeys)
        {
            ...
            if (FactorKeySet.Start.Factor.Contains(currKey))
            {
                switch (currKey)
                {
                    ...
                    case Key.ERGraph:
                        Accept(Key.ERGraph);
                        resultStatList.AddRange(RandomGraph.Parse(endKeys, Builder.Structure.BuildExpr.Stack.PushNewErdosRenyiGraph));
                        break;
                }
            }
