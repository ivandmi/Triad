    ...
    class IProcedureCodeBuilder : RoutineCodeBuilder
    {
        ...
        public void AddSpyObject(ISpyType spyObjectType)
        {
            ...
            if (spyObjectType is IExprType)
            {
                ...
                //так надо, но возможно надо переделать
                AddGetMethod(spyObjectType as IExprType);
            }
        }
        ...
        private void AddGetMethod(IExprType spyVarType)
        {
            string typeStr = GetBaseTypeString(spyVarType);

            CodeMemberMethod method =new CodeMemberMethod();
            method.Name="get"+ spyVarType.Name;
            method.ReturnType = new CodeTypeReference(typeStr);
            ...
            CodeParameterDeclarationExpression c = new CodeParameterDeclarationExpression();
            c.Name = "indexList";
            c.Type = new CodeTypeReference("params int[]");
            method.Parameters.Add(c);
            //Аксептор Get
            CodeMethodReturnStatement getValueStat = new CodeMethodReturnStatement(
                new CodeSnippetExpression(
                "((" + typeStr + ")" + Builder.IProcedure.GetValueForVar +
                "(new " + Builder.CoreName.Name + "(\"" + spyVarType.Name + "\", indexList)))"));
            method.Statements.Add(getValueStat);
            //property.GetStatements.Add(getValueStat);
