...
namespace TriadNSim.Forms
{
    public partial class frmCalculate : Form
    {
        ...
        private void ToExcel()
        {
            ...
            oExcelApplication.SheetsInNewWorkbook = 5;
            //oExcelApplication.Columns.EntireColumn.AutoFit();
            ...
            if (checkOverallProperties.Checked)
            {
                ...
                oSheet.Cells[2, 2] = lstLinks.Count; //Можно считать как половину суммы всех степеней
            ...
            if (!IsAllVertexPropertiesUnselected())
            {
                ...
                for (int i = 0; i < lstObjects.Count; i++)
                {
                    ...
                    if (iClusteringCoeffCol > 0)
                        oSheet.Cells[2 + i, iClusteringCoeffCol] = Math.Round(oClusteringCoefficients[new CoreName(lstObjects[i].Name)],2);
                    if (iDegreeCentralityCol > 0)
                    {
                        ...
                        oSheet.Cells[2 + i, iDegreeCentralityCol] = Math.Round(dDegreeCentrality,2);
                    }
                    if (iBetweennessCentralityCol > 0)
                        oSheet.Cells[2 + i, iBetweennessCentralityCol] = Math.Round(oBetweennessCentralities[new CoreName(lstObjects[i].Name)],2);
                    if (iClosenessCentralityCol > 0)
                        oSheet.Cells[2 + i, iClosenessCentralityCol] = Math.Round(oClosenessCentralities[new CoreName(lstObjects[i].Name)],2);
                    if (iEigenvectorCentralityCol > 0)
                        oSheet.Cells[2 + i, iEigenvectorCentralityCol] = Math.Round(oEigenvectorCentralities[new CoreName(lstObjects[i].Name)],2);
                    if (iSccCol > 0)
