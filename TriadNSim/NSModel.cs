            ...
            foreach (NetworkObject node in SimInfo.Nodes)
            {
                ...
                else
                {
					...
					if (node.Type == ENetworkObjectType.UserObject)
                        ...
                        //sPath = "D:\\Учеба\\Курсовая\\Версии Triad\\TriadNS_2\\TriadNS\\bin\\Release\\" + node.Routine.Name + ".dll";
					else
                ...
                //костыль для индексированных имен вершин
                CoreName cr;
                if (node.Name.Contains('['))
                {
                    string[] m = node.Name.Split('[', ']');
                    cr = new CoreName(m[0], int.Parse(m[1]));
                }
                else
                    cr = new CoreName(node.Name);
                model.RegisterRoutine(cr, routine);
            }
